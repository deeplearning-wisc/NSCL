...
def main(log_writer, log_file, device, args):
    ...
    for epoch in range(0, args.train.stop_at_epoch):
        ...
        for idx, ((_, _), target) in enumerate(train_label_loader):
            ((ux1, ux2), target_unlabeled) = next(unlabel_loader_iter)
            ux1, ux2, target, target_unlabeled =  ux1.to(device), ux2.to(device), target.to(device), target_unlabeled.to(device)

            model.zero_grad()
            data_dict = model.forward_uonly(ux1, ux2)
